...
shifts = np.arange(7, 20)
orig_energies = np.zeros(len(shifts))
sc_energies = np.zeros(len(shifts))
real_dists = np.zeros(len(shifts))
...
real_dists = real_dists*0.529177210903
# %%
...
plt.figure()
plt.plot(real_dists, orig_energies, '.', label='Trial wave function')
plt.plot(real_dists, sc_energies, '.', label='Optimized wave function')
plt.xlabel(r'Interatomic distance ($\AA$)')
plt.ylabel('Energy ($Ry$)')
plt.grid()
plt.legend()
plt.tight_layout()
plt.savefig(img_loc('8c1.png'))
plt.show()

print(real_dists[np.argmin(orig_energies)])
print(real_dists[np.argmin(sc_energies)])
print(np.min(orig_energies))
print(np.min(sc_energies))
